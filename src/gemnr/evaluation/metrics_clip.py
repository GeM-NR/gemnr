...
class ClipSimilarity(torch.nn.Module):
    def __init__(self, name: str = "ViT-L/14", device: str = "cuda"):
        ...
        self.model, _ = clip.load(
            name,
            device=device,
            download_root=os.environ.get("TORCH_HOME", None),
        )
    ...
    def encode_text(self, text):
        text = clip.tokenize(text, truncate=True).to(
    ...
    def encode_img_paths(self, img_paths, seed: int = 0):
        torch.manual_seed(seed)
        imgs = torch.stack(
            [ToTensor()(Image.open(p).resize((512, 512))) for p in img_paths]
        ).to(
            self.device
        )  # [N, C, H, W]
    ...
    def forward(self, image_0, image_1, text_0, text_1, seed: int = 0):
        torch.manual_seed(seed)
    ...
    def comp_image_text(self, image, text, device="cuda"):
